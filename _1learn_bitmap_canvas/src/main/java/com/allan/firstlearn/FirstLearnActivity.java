...
public class FirstLearnActivity extends Activity {
    ...
    RoundRayProgressView mProgressRay;
    CircleImageView helpImageView;
    @Override
    protected void onCreate(Bundle savedInstanceState) {
        super.onCreate(savedInstanceState);
        setContentView(R.layout.firstlearn_main);
        mProgressRay = findViewById(R.id.fr_temp_id1);
        helpImageView = findViewById(R.id.helpview);
        helpImageView.setImageResource(R.drawable.left);
    }

    public void onClickedLoad(View view) {
        if (true) {
            AnimHelper animHelper = new AnimHelper();
            animHelper.init(helpImageView);
            animHelper.start();
            return;
        }
