...
public class MyAudioTracker {
    ...
    private void playInStatic(Context context, final String filePath) {
            ...
            public void run() {
                ...
                try (ByteArrayOutputStream out = new ByteArrayOutputStream(12*1024);
                    ...
                    audioTrack = new AudioTrack(AudioManager.STREAM_MUSIC, pcm.getSampleRate(),
                            pcm.getOutChannelConfig(), pcm.getEncodingFmt(),
    ...
    private void playInStream(Context context, final String filePath) {
        final SoftReference<Context> sf = new SoftReference<>(context);
        ThreadPoolUtils.getThreadPollProxy().execute(new Runnable() {
            @Override
            public void run() {
                ...
                audioTrack = new AudioTrack(
                        ...
                        new AudioFormat.Builder().setSampleRate(pcm.getSampleRate())
                                ...
                                .setEncoding(pcm.getEncodingFmt())
                                .build(),
                        minBufferSize,
    ...
    public int play(Context context, final String filePath) {
        ...
        if (file.exists()) {
            ...
            if (file.length() > LENGTH_OF_STREAM) {
                MyLog.d("走Stream Mod~~");
                playInStream(context, filePath);
            } else {
                MyLog.d("走Static Mod！");
                isStatic = true;
                playInStatic(context, filePath);
            }
        }
